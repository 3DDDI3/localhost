<section class="news__slider splide">
    <div class="splide__track">
        <ul class="splide__list">
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news1.png') }}" alt="" date="15.04.2024"
                    title="Авиакомпания Utair повезет туристов в Анталью и Сочи" />
            </li>
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news2.png') }}" alt="" date="11.04.2024"
                    title="Белгородский центр туризма получил Гран-при" />
            </li>
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news3.png') }}" alt="" date="11.04.2024"
                    title="На границе с Китаем в сумке туриста нашли зубы кашалота" />
            </li>
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news1.png') }}" alt="" date="15.04.2024"
                    title="Авиакомпания Utair повезет туристов в Анталью и Сочи" />
            </li>
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news2.png') }}" alt="" date="11.04.2024"
                    title="Белгородский центр туризма получил Гран-при" />
            </li>
            <li class="splide__slide">
                <x-templates.news url="" img="{{ asset('images/news3.png') }}" alt="" date="11.04.2024"
                    title="На границе с Китаем в сумке туриста нашли зубы кашалота" />
            </li>
        </ul>
    </div>

</section>
